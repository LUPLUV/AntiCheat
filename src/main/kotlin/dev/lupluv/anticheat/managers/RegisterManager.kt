package dev.lupluv.anticheat.managers

import dev.lupluv.anticheat.AntiCheat
...
object RegisterManager {
     ...
     */
    fun registerAll() {
        val reflections = Reflections("dev.lupluv.anticheat")
        ...
        val timeListeners = measureTimeMillis {
            for (clazz in reflections.getSubTypesOf(Listener::class.java)) {
                try {
                    ...
                    Bukkit.getPluginManager().registerEvents(event, AntiCheat.instance)
        ...
        val timeCommands = measureTimeMillis {
            for (clazz in reflections.getTypesAnnotatedWith(RegisterCommand::class.java)) {
                try {
                    ...
                    val command: PluginCommand = constructor.newInstance(annotation.name, AntiCheat.instance)
                    ...
                    Bukkit.getCommandMap().register(AntiCheat.instance.name.lowercase(), command)
